...
store.getAll = async function({ host=config.host, port=config.port, db=config.db, table, fields=[] }) {
  const conn = await store.getConn({ host, port, db });

  const query = queryHelper.pluck({ query: r.table(table), fields });
...
store.betweenByIndex = async function({ host=config.host, port=config.port, db=config.db, table, index, from, to, page=1, size=50, fields=[] }) {
  const conn = await store.getConn({ host, port, db });
